""" Code to implement a school management system application with a graphical user interface (GUI) using Tkinter
"""
...
connector = sqlite3.connect("SchoolManagement.db")  # Connecting to a SQLite database
cursor = connector.cursor()

"""Creating a table named "SCHOOL_MANAGEMENT" to store information about students such as
        ID (auto-generated), name, email, phone number, gender, date of birth, and stream.
"""
connector.execute(
...
def reset_fields():
    """Clears all the entry fields and resets the date to the current date"""
...
def reset_form():
    """Clears all displayed records in the treeviews"""
    global tree
...
def display_records():
    """Fetches all records from the databases and displays in the treeview"""
    tree.delete(*tree.get_children())
...
def add_record():
    """Adds a new student record to the database and displays a success message.
    Clears entry fields and refreshes the treeview.
    """
    global name_strvar, email_strvar, contact_strvar, gender_strvar, dob, stream_strvar
...
def remove_record():
    """Checks if a student record is selected in the treeview. If selected, removes
    the reord from the database and refreshes the treeview.
    """
...
def view_record():
    """Fetches the data of the selected student record and populates the entry fields with the retrieved information"""
    global name_strvar, email_strvar, contact_strvar, gender_strvar, dob, stream_strvar
...
Label(main, text="SCHOOL MANAGEMENT SYSTEM", font=headlabelfont, bg="SpringGreen").pack(
    side=TOP, fill=X
)

"""  The window is divided into three frames: left, center, and right.  """
left_frame = Frame(
    main, bg=lf_bg
)  # Left frame contains labels and entry fields for user input
left_frame.place(x=0, y=30, relheight=1, relwidth=0.2)

center_frame = Frame(
    main, bg=cf_bg
)  # Center frame contains buttons for various actions: Delete Record, View Record, etc.
center_frame.place(relx=0.2, y=30, relheight=1, relwidth=0.2)

right_frame = Frame(
    main, bg="Gray35"
)  # Right frame displays the list of student records in a treeview widget
...
Button(
    center_frame, text="Delete Record", font=labelfont, command=remove_record, width=15
).place(relx=0.1, rely=0.25)
Button(
    center_frame, text="View Record", font=labelfont, command=view_record, width=15
